...
// Robot radius: 96.896 mm
#define WHEEL_COUNT         3
#define WHEEL_RADIUS        0.019
#define ROBOT_RADIUS        0.096896

class MadBotController : public rclcpp::Node
{
public:
  MadBotController(int num_wheels_, double robot_radius_, double wheel_radius_, double heading_offset_ = 0)
  : Node("madbot_controller_node")
  {
    ...
    pub_all_wheels = this->create_publisher<std_msgs::msg::Float64MultiArray>("wheels_controller/commands", 10);

    // pub_odometry = this->create_publisher<nav_msgs::msg::Odometry>("odom", 10);

    sub_cmd_vel = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel", 10, std::bind(&MadBotController::cmd_vel_callback, this, _1));
    // sub_join_states = this->create_subscription<sensor_msgs::msg::JointState>("joint_states", 10, std::bind(&MadBotController::join_states_callback, this, _1));
    // sub_imu = this->create_subscription<sensor_msgs::msg::Imu>("imu", 10, std::bind(&MadBotController::imu_callback, this, _1));
  ...
  ~MadBotController() {
    cout << "MadBotController Destroyed" << endl;
  }

private:
  rclcpp::TimerBase::SharedPtr timer_;
  rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr pub_all_wheels;
  ...
  void set_all_motors_speed(Eigen::VectorXd M) {
    for(int i = 0; i < M.size(); i++) {
      RCLCPP_INFO(this->get_logger(), "Setting motor %d speed to %f", i, M(i));
    }
    auto message = std_msgs::msg::Float64MultiArray();
    message.data = {M(0), M(1), M(2)};
    pub_all_wheels->publish(message);

    cout << "Eigen::VectorXd M: " << M << endl;
  }
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<MadBotController>(WHEEL_COUNT, ROBOT_RADIUS, WHEEL_RADIUS));
  rclcpp::shutdown();
  return 0;
}
